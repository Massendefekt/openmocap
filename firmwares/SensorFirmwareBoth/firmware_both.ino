...
*   
*    --------------------------------------------------------------------------------------------------------------
*    | Sensor Identifier | AX | AY | AZ | GX | GY | GZ | MX | MY | MZ | W | X | Y | Z | Generation (packet number)|
*    --------------------------------------------------------------------------------------------------------------
...
*
         W..Z are the parts of the orientation quaternion calculated by the sensor
...
void loop(void) {
    ...
    Serial.println();
    */
    sensor_packet_t packet = {.id = sensor_id, .ax = round_to_int(acc.x(), 4.0), .ay = round_to_int(acc.y(), 4.0), .az = round_to_int(acc.z(), 4.0), .gx = round_to_int(gyr.x(), 4.0), .gy = round_to_int(gyr.y(), 4.0), .gz = round_to_int(gyr.z(), 4.0), .mx = round_to_int(mag.x(), 4.0), .my = round_to_int(mag.y(), 4.0), .mz = round_to_int(mag.z(), 4.0), .x = round_to_int(euler.x(), 4.0), .y = round_to_int(euler.y(), 4.0), .z = round_to_int(euler.z(), 4.0), .generation = generation};
    ...
    top = top + 5;
    push(packet.x, buffer, top);
    top = top + 5;
    push(packet.y, buffer, top);
    top = top + 5;
    push(packet.z, buffer, top);
    top = top + 5;
